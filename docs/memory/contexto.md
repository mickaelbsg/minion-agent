...
- A instalação usa `apt` sobre um `.deb` local (`sudo apt install ./minion_<versao>_amd64.deb`); não há atualmente um repositório APT remoto publicado para instalar com `apt install minion`.
...
- O `postinst` deve arquivar uma unit legada em `/etc/systemd/system/minion.service`, ativar a unit empacotada e aguardar `/api/v1/health` antes de declarar a instalação concluída.
